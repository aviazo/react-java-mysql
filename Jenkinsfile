  ...
  stages {  
    ...
    stage('Build and Push To Nexus Image') {
      steps {
	     ...
	     sh '''docker tag react-java-mysql_frontend:latest react-java-mysql-frontend:${BUILD_ID}'''
             sh '''docker tag react-java-mysql-backend:latest react-java-mysql-backend:${BUILD_ID}'''	     
